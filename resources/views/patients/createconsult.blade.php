							...
							<div class="box-body">
								<div class="row">
									<div class="col-md-4">
													...
													<a href="{{URL::route('patients.createconsult',['id'=>$patient->id,'repeatvisitid'=>$visit->id])}}"  class="btn btn-xs btn-warning">Repeat All</a> <span>Using this would refresh all</span>
												</div>
											</div>
									...
									$.ajax({
										...
										success: function(response){
											...
											for (var i = 0; i < response['pathology'].length; i++) {
												 $option.push({id:response['pathology'][i]['id'],a_key:response['pathology'][i]['name']});
												//$option.push({id:response['pathology'][i]['id']});
											}
											console.log($option);
											//$('#pathology').val($option).trigger('change');
										 	// $('#pathology').select2({
										 	// 	data:$option});
										 	

										 	$('#pathology').select2('data':{$option});
